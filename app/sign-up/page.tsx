        ...
        <div className="min-h-screen flex items-center justify-center relative">
            ...
            <div className="auth-card w-full max-w-md mx-4 p-10 relative z-10 animate-scale-in">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    {successMsg && (
                        <div className="p-3 bg-green-500/10 border border-green-500/20 text-green-400 text-sm rounded-xl font-medium">
                            {successMsg}
                        </div>
                    )}

                    <button type="submit" disabled={isLoading} className="btn-primary w-full mt-2 flex justify-center items-center gap-2">
                        {isLoading ? <Loader2 className="animate-spin w-5 h-5" /> : "Create an Account"}
                    </button>
                </form>

                <p className="text-center text-sm text-white/50 mt-6">
                    Have an account already?{" "}
                    <Link href="/sign-in" className="text-white font-semibold hover:text-[var(--primary)] transition-colors">
                        Sign In
                    </Link>
                </p>
            </div>
